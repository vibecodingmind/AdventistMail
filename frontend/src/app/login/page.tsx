  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const res = await api<{ accessToken: string; refreshToken: string; is_new?: boolean }>('/auth/google', {
          method: 'POST',
        ...
        <div className="flex-1 bg-[#0E1829] flex flex-col justify-center px-10 py-12">
          ...
          {hasGoogleClientId && (
            <button
              ...
              className="w-full py-3 flex items-center justify-center gap-3 border border-white/20 rounded-lg text-sm font-semibold text-white/70 hover:border-white/40 hover:text-white transition-colors disabled:opacity-50 mb-3"
            >
